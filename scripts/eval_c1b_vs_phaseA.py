#!/usr/bin/env python3
"""C1b frozen-teacher vs Phase A: greedy val 200 + 2-ply play-out n=20."""
...
OUT = Path("/root/autodl-tmp/RL-2048/results/experiments/c1b_vs_phaseA.json")
C1B_DIR = Path(
    "/root/autodl-tmp/RL-2048/results/runs/c1b_frozen_teacher/c1b_frozen_teacher_seed0_20260818T005854"
)
PHASE_A = Path("checkpoints/phaseA_dueling_seed0.pt")
GREEDY_CKPTS = [
    ("phaseA", PHASE_A),
    ("c1b_100k", C1B_DIR / "checkpoint_100000.pt"),
    ("c1b_final", C1B_DIR / "checkpoint_final.pt"),
]
...
def main() -> None:
    ...
    payload = {
        ...
        "greedy": [],
        "playout_2ply": {},
        "phaseA_2ply_reference": {
            "n": 20,
            "mean_score": 50522.8,
            "p_reach_2048": 1.0,
            "p_reach_4096": 0.55,
            "note": "same val_seeds(20), play-out max_steps=8000, from prior eval",
        },
    }
    greedy_seeds = val_seeds(200)
    print(f"=== greedy val {len(greedy_seeds)} max_steps=4000 ===", flush=True)
    for name, path in GREEDY_CKPTS:
        if not path.exists():
    ...
    path = C1B_DIR / "checkpoint_final.pt"
    policy = ExpectimaxDQNPolicy.from_checkpoint(
        path, depth=2, adaptive=False, corner_tiebreak=True
    )
    rows = playout_rows(policy, play_seeds, key="c1b_final_d2", max_steps=8000)
    payload["playout_2ply"]["c1b_final"] = summarize_playout("c1b_final", rows)
    summary = payload["playout_2ply"]["c1b_final"]
    print(
        f"  DONE c1b_final mean={summary['mean_score']:.0f} "
